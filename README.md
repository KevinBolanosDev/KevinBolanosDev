<h1 align="center">Hi <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="40" heigth="40"> I'm Kevin Andres Bolaños Narvaez</h1>
<h2 align="center">Full Stack Developer</h2>

<div>
<h3 align="center">Frontend developer focused on creating modern, scalable, and user-centric web applications.
  
I have experience working with the following frontend technologies and tools: React.js/Next.js, Tailwind CSS, Shadcn, ChakraUI, Bootstrap, Tanstack Query, Zuatand, Axios, and Zod, using architectures like FSD to build efficient and maintainable interfaces.

For backend development, I am proficient with Node.js, NestJS, Supabase, Primsa, Zod, Pino, MongoDB, and Docker.

For CI/CD deployments, I use Vercel, Railway, Supabase Cloud, GitHub Actions, Netlify, and Render.

I am a proactive, disciplined, and highly skilled problem solver, always committed to continuous learning and improving my abilities.</h3>
</div>

---
<h3 align="center">Projects</h3> 

<div style="display: flex; gap: 4">
<div align="center">
  Frontend Developer | Lead Management Platform
  <div>
    <a href="https://ariainsurtech.com">Production</a>
  </div>
  <div>
  <img src="https://github.com/KevinBolanosDev/KevinBolanosDev/blob/main/AriaInsurtech-home.png">
  </div>
</div>

<div align="center">
  Create a Web App on a task list, using React and ChakraUI.
  <div>
  <a href="https://aria-desk.com/home">Production</a>
  </div>
  <div>
  <img src="https://github.com/KevinBolanosDev/personal-cv/blob/main/img/react-task-list.png?raw=true" width="250" heigth="250">
  </div>
</div>

---

<h3 align="center">Personal practice projects</h3>

<div align="center">
  Develop a front-end design taken from Frontend Mentor.
  <div>
  <a href="https://github.com/KevinBolanosDev/news-homepage-frontend-mentor">Code Github</a> /
  <a href="https://news-homepage-mentor-solution.netlify.app">Preview Project</a>
  </div>
  <div>
  <img src="https://github.com/KevinBolanosDev/personal-cv/blob/main/img/news-homepage.png?raw=true" width="250" heigth="250">
  </div>
</div>
</div>

---
<h3>Connect with me:</h3>
**im.kevinbolanos.dev@gmail.com**
<a href="https://www.imkevinbolanosdev.com/home">Portfolio</a>
